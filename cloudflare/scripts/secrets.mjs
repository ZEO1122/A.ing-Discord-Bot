...
try {
  ...
  if (!discordOnly && !process.env.OPENAI_API_KEY) throw new Error("missing_key");
  ...
  child.stdin.end(JSON.stringify(discordOnly ? {DISCORD_WEBHOOK_URL: process.env.DISCORD_WEBHOOK_URL} : { OPENAI_API_KEY: process.env.OPENAI_API_KEY, ADMIN_TOKEN: token }));
  const code = await new Promise((resolve, reject) => { child.on("error", reject); child.on("exit", resolve); });
  if (code !== 0) throw new Error("upload_failed");
  console.log(discordOnly ? "Cloudflare Secrets configured: DISCORD_WEBHOOK_URL (value hidden)" : "Cloudflare Secrets configured: OPENAI_API_KEY, ADMIN_TOKEN (values hidden)");
} catch {
