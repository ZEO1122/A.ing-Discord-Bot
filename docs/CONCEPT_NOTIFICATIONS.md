...
12주 × 주 3회, 총 36편의 Markdown 초안과 선수 개념 그래프를 작성했다.
사용자 피드백에 따라 글머리 기호와 개념 설명·수치 사례 중심으로 구성했다.
모든 콘텐츠는 운영 승인 전인 draft 상태이며, 전체 과정의 자동 운영 준비 완료를 뜻하지 않는다.
[주차별 목차](../content/dl-foundations/README.md)와 [집필 검증 기록](CONCEPT_CONTENT_REVIEW.md)을 참고한다.

사용자가 확정한 시각은 월·수·금 09:00 Asia/Seoul이다. 학기 시작일과 시험 기간은
미정이다. config/semesters/example.yaml의 시작일은 테스트 예시일 뿐이며 자동 예약은
등록하지 않았다. 뉴스와 개념 채널을 분리했고 개념은 CONCEPT_WEBHOOK_URL만 사용한다.
...
현재 36편 모두 작성했지만 운영 승인 전이므로 이 명령이 실패하는 것이 정상이다. 개발 빌드에는 draft를 담을 수
...
36편의 문구·학습 흐름을 최종 검토하고 승인한다. 이후
학기 시작일·시험 기간 확정, 스케줄 활성화·복구 도구 보완 및 실제 Cron
